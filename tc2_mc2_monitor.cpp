...
bool struct_field_offset(const tc2_mc2::TypeRegistry& registry,
                         const tc2_mc2::TypeDef& def,
                         ...
                         size_t& out_offset)
{
  ...
  for (const auto& f : def.fields) {
    ...
    if (def.kind == tc2_mc2::TypeDef::Kind::Union) {
      if (f.name == field) {
        out_offset = 0;
        return true;
      }
...
size_t struct_size_with_pack(const tc2_mc2::TypeRegistry& registry,
                             const tc2_mc2::TypeDef& def,
                             size_t pack)
{
  size_t offset = 0;
...
bool struct_field_offset_with_pack(const tc2_mc2::TypeRegistry& registry,
                                   const tc2_mc2::TypeDef& def,
                                   const std::string& field,
                                   size_t pack,
                                   size_t& out_offset)
...
size_t pick_pack_for_struct(const tc2_mc2::TypeRegistry& registry,
                            const tc2_mc2::TypeDef& def,
                            size_t ads_size)
{
...
AdsLayoutInfo get_ads_layout(const tc2_mc2::TypeRegistry& registry)
{
  AdsLayoutInfo info;
...
size_t effective_size(const tc2_mc2::TypeRegistry& registry,
...
void Tc2Mc2Monitor::load(const std::string& root)
{
  registry_.load(root);
}

void Tc2Mc2Monitor::clear(size_t symbol_count)
{
  struct_children_items_.assign(symbol_count, {});
}

bool Tc2Mc2Monitor::shouldSkipSymbol(const std::vector<SymbolEntry>& syms,
...
bool Tc2Mc2Monitor::addChildrenForSymbol(const std::vector<SymbolEntry>& syms,
                                         const std::unordered_map<std::string, int>& name_to_index,
                                         int sym_index,
                                         QTreeWidgetItem* item)
{
  ...
  if (def && (def->kind == tc2_mc2::TypeDef::Kind::Struct || def->kind == tc2_mc2::TypeDef::Kind::Union) &&
      norm_type != "AXIS_REF") {
    item->setFlags(item->flags() & ~Qt::ItemIsEditable);
  ...
  if (type == "AXIS_REF" || type.find("AXIS_REF") != std::string::npos) {
    ...
    if (plc_def) {
      ...
      if (plc_to_nc->childCount() == 0) {
        const auto fallback = registry_.fallbackPlcToNc();
        const size_t pack = pick_pack_for_struct(registry_, fallback, plc_source_size);
        addStructChildren(plc_to_nc, plc_has_symbol ? 0 : plc_base, plc_source,
                          plc_source_size,
                          fallback,
                          true,
    ...
    if (nc_def) {
      ...
      if (nc_to_plc->childCount() == 0) {
        const auto fallback = registry_.fallbackNcToPlc();
        const size_t pack = pick_pack_for_struct(registry_, fallback, nc_source_size);
        addStructChildren(nc_to_plc, nc_has_symbol ? 0 : nc_base, nc_source,
                          nc_source_size,
                          fallback,
                          true,
    ...
    if (ads_def && ads->childCount() == 0) {
      ...
      addStructChildren(ads, ads_has_symbol ? 0 : ads_base, ads_source,
                        ...
                        *ads_def,
                        ...
                        pack);
    } else if (!ads_def && ads->childCount() == 0) {
      const auto fallback = registry_.fallbackAdsAddress();
      const size_t pack = pick_pack_for_struct(registry_, fallback, ads_source_size);
      addStructChildren(ads, ads_has_symbol ? 0 : ads_base, ads_source,
                        ads_source_size,
                        fallback,
                        true,
...
void Tc2Mc2Monitor::addStructChildren(QTreeWidgetItem* parent,
                                      size_t base_offset,
                                      int source_index,
                                      size_t source_size,
                                      const tc2_mc2::TypeDef& def,
                                      ...
                                      size_t pack)
{
  ...
  for (const auto& f : def.fields) {
    ...
    const bool is_complex = field_def && (field_def->kind == tc2_mc2::TypeDef::Kind::Struct ||
                                          field_def->kind == tc2_mc2::TypeDef::Kind::Union);
    ...
    if (f.hidden) {
      ...
      } else if (def.kind != tc2_mc2::TypeDef::Kind::Union) {
        if (bit_index > 0) {
    ...
    if (!is_bit && def.kind != tc2_mc2::TypeDef::Kind::Union) {
      if (bit_index > 0) {
        offset += (bit_index + 7) / 8;
        bit_index = 0;
      }
    ...
    if (bind_values && def.kind != tc2_mc2::TypeDef::Kind::Union && base_offset + offset >= source_size) {
      break;
    }
    ...
    for (size_t idx = 0; idx < field_array_len; ++idx) {
      ...
      if (is_bit) {
        ...
        if (bind_values) {
          child->setFlags(child->flags() | Qt::ItemIsEditable);
          const size_t field_offset = (def.kind == tc2_mc2::TypeDef::Kind::Union) ? 0 : offset;
        ...
        if (bind_values && def.kind != tc2_mc2::TypeDef::Kind::Union &&
            base_offset + offset + idx * elem_size + elem_size > source_size) {
          break;
        }
        child->setFlags(child->flags() & ~Qt::ItemIsEditable);
        const size_t field_offset = (def.kind == tc2_mc2::TypeDef::Kind::Union) ? 0 : (offset + idx * elem_size);
        addStructChildren(child, base_offset + field_offset, source_index, source_size, *field_def, bind_values, group_tag, pack);
      } else {
        if (bind_values && def.kind != tc2_mc2::TypeDef::Kind::Union &&
            base_offset + offset + idx * elem_size + elem_size > source_size) {
          break;
...
void Tc2Mc2Monitor::maybeFixAxisAdsOffsets(const std::vector<std::vector<uint8_t>>& raw)
{
...
void Tc2Mc2Monitor::updateValues(const std::vector<std::vector<uint8_t>>& raw, PlcClient& client)
{
...
bool Tc2Mc2Monitor::isStructItem(QTreeWidgetItem* item) const
{
  return item && item->data(0, kStructRole).toBool();
}

std::string Tc2Mc2Monitor::resolveTypeName(const std::string& type) const
{
  return registry_.resolveAlias(type);
}

bool Tc2Mc2Monitor::handleEdit(QTreeWidgetItem* item,
                               PlcClient& client,
...
size_t Tc2Mc2Monitor::typeSize(const std::string& type) const
{

...
7. FB\_DALI209QuerySceneValues

# FB\_DALI209QuerySceneValues

![7258393](/tcplclib_tc3_dali/1033/Images/gif/9043489931__Web.gif)

The function block reads the output values for the specified scene *nScene* from the DALI control gear.

Initially, only the color temperature is readable.

|  |  |
| --- | --- |
| 65325544 | In addition to the DALI command QUERY SCENE LEVEL, the function block sends the DALI commands QUERY COLOUR VALUE and ENABLE DEVICE TYPE 8, as is necessary for application-related extension commands (see [FB\_DALI102EnableDeviceType](ms-xhelp:///?Id=beckhoff-6f87-4d1b-b5a6-4f03ac5df1fb)). |

|  |  |
| --- | --- |
| 51058985 | The function block changes the DTR0 of all connected DALI control gears. |
...
|  |  |
| --- | --- |
| 56877687 | *eCommandPriority* has no function with the KL6811. The command priorities are supported by the KL6821 from the firmware version BD. |

**nScene:** Scene whose output value is to be read (0…15).

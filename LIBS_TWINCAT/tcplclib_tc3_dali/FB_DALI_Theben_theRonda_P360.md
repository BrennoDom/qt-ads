...
6. FB\_DALI\_Theben\_theRonda\_P360

# FB\_DALI\_Theben\_theRonda\_P360

![24005299](/tcplclib_tc3_dali/1033/Images/gif/9007208340816523__Web.gif)

The function block represents theRonda P360 DALI sensor from Theben.

This outputs one occupancy signal (instance 0), four measured brightness values (instances 1 to 4) and eight push-button messages (instances 5 to 12).
...
Readout can be triggered manually via the input variables *bQueryOccupancy, bQueryBrightnessIntegral*, *bQueryBrightnessInside*, *bQueryBrightnessCenter*, and *bQueryBrightnessWindow*.

|  |  |
| --- | --- |
| 14721100 | Set the times so that as few events as possible are sent. If too many events are sent, this can have a negative effect on the behavior of the application. |

|  |  |
| --- | --- |
| 65381042 | The function block cannot be used when using the KL6811. |
...
|  |  |
| --- | --- |
| 51558469 | These measured values must be compared with the measured reference values at the place of operation of the DALI control device. |
...
| bEnableOccupancy | BOOL | Get, Set | TRUE | TRUE at this property enables the instance for motion detection. FALSE at this property causes sending of events from this instance to be disabled. However, the actual value of the instance can still be queried via the input *bQueryOccupancyN*. |

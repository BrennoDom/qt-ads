# FB_DALI253QueryLightSourceStartCounter
...
6. FB\_DALI253QueryLightSourceStartCounter

# FB\_DALI253QueryLightSourceStartCounter

![59735509](/tcplclib_tc3_dali/1033/Images/gif/8371596939__Web.gif)

The function block reads the non-resettable counter for the switching on of the light source. The non-resettable counter is incremented by one each time the light source changes from the Off state to the On state. The non-resettable counter cannot be reset or changed.
...
|  |  |
| --- | --- |
| 748676 | *eCommandPriority* has no function with the KL6811. The command priorities are supported by the KL6821 from the firmware version BD. |
...
bBusy                                       : BOOL;  
nLightSourceStartCounter                    : UDINT;
```
...
**nLightSourceStartCounter:** Current value of the non-resettable counter for the switching on of the light source (0...16777213).

...
5. FB\_DALI303OccupancySensor

# FB\_DALI303OccupancySensor

![24537816](/tcplclib_tc3_dali/1033/Images/gif/27021605507079435__Web.gif)
...
|  |  |
| --- | --- |
| 19513753 | Set the times so that as few events as possible are sent. If too many events are sent, this can have a negative effect on the behavior of the application. |

|  |  |
| --- | --- |
| 41406050 | The function block cannot be used when using the KL6811. |
...
**bCancelHoldTimer:** A positive edge at this input prematurely terminates the Hold Timer and resets the output *bOccupied*.
...
| nEventFilter | BYTE | Get, Set | 2#0000\_0111 | Setting of the [event filter](ms-xhelp:///?Id=beckhoff-5860-45ad-91f4-6edf1ff97245). |
...
| nHoldTimer | UINT | Get, Set | 900 s | Dwell time (1...2538 s) in which the occupancy continues to be active after no more movement has been detected by the sensor. |
| nReportTimer | BYTE | Get, Set | 60 s | Time (0…255 s) after which the pending event of the occupancy sensor is repeated if no other event has occurred in the meantime. |

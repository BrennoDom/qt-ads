...
7. FB\_DALI102SetShortAddress

# FB\_DALI102SetShortAddress

![9875225](/tcplclib_tc3_dali/1033/Images/gif/9007205111222795__Web.gif)

The function block stores the new short address *nNewShortAddress* in the [shortAddress](ms-xhelp:///?Id=beckhoff-e013-4a7e-9d50-4deaaa138af7) variable of the DALI control gear.

The existing short address in the DALI control gear is deleted if a value of 255 is entered at the input.

If you want to give a short address to a device that does not yet have one, you must transmit the command as a broadcast (*eAddressType* = E\_DALIAddressType.Broadcast). This gives all DALI control gears connected to the DALI terminal the short address *nNewShortAddress.*

Alternatively, the function block can be called with Broadcast unaddressed (*eAddressType* = E\_DALIAddressType.BroadcastUnaddr). In this case, only the DALI devices that do not yet have a short address are assigned a short address.

|  |  |
| --- | --- |
| 21768161 | The function block changes the DTR0 of all connected DALI control gears. |

|  |  |
| --- | --- |
| 61695723 | The function block sends the DALI command SET SHORT ADDRESS twice, as required by the DALI standard for certain commands. |
...
|  |  |
| --- | --- |
| 18390601 | *eCommandPriority* has no function with the KL6811. The command priorities are supported by the KL6821 from the firmware version BD. |

**nNewShortAddress:** New short address (0…63, 255).

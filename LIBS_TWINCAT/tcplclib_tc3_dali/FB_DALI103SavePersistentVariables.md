# FB_DALI103SavePersistentVariables
...
7. FB\_DALI103SavePersistentVariables

# FB\_DALI103SavePersistentVariables

![64964848](/tcplclib_tc3_dali/1033/Images/gif/9007205204426379__Web.gif)

The function block immediately triggers the saving of variables in the persistent memory by the DALI command SAVE PRESISTENT VARIABLES.

DALI control devices do not save the variables directly in the persistent memory. Instead, the values are copied to the persistent memory in the background. Since this happens over a longer period of time (a few seconds), the values can be lost if the DALI bus is disconnected.

The SAVE PRESISTENT VARIABLES DALI command triggers immediate copying to the persistent memory. In most cases, no communication with the DALI device can take place during copying (up to 350 ms).

|  |  |
| --- | --- |
| 47812724 | The function block sends the DALI command SAVE PRESISTENT VARIABLES twice, as required by the DALI standard for certain commands. |

|  |  |
| --- | --- |
| 27661336 | The function block cannot be used when using the KL6811. |
...
|  |  |
| --- | --- |
| 47625437 | The command priorities are supported by the KL6821 from the firmware version BD. |

...
7. FB\_DALI103SetEventFilter

# FB\_DALI103SetEventFilter

![25975757](/tcplclib_tc3_dali/1033/Images/gif/9007205112180235__Web.gif)

The function block sets the event filter (see [eventFilter](ms-xhelp:///?Id=beckhoff-708d-47aa-9cfa-2706c5d5c470)) for the corresponding instance of the DALI control device.

Each bit of the input variable *nEventFilter* represents an event. If the bit is set, the associated event is enabled.

The meanings of the individual bits for the respective instance types can be found here:
...
|  |  |
| --- | --- |
| 32455223 | The function block changes the DTR0, DTR1 and DTR2 of all connected DALI control devices. |

|  |  |
| --- | --- |
| 23661558 | The function block sends the DALI command SET EVENT FILTER twice, as required by the DALI standard for certain commands. |

|  |  |
| --- | --- |
| 11627434 | The function block cannot be used when using the KL6811. |
...
|  |  |
| --- | --- |
| 37538044 | The command priorities are supported by the KL6821 from the firmware version BD. |

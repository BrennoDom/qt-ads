# FB_DALI208SetUpSwitchOnThreshold
...
7. FB\_DALI208SetUpSwitchOnThreshold

# FB\_DALI208SetUpSwitchOnThreshold

![40002310](/tcplclib_tc3_dali/1033/Images/gif/10380442891__Web.gif)

This function block saves the value *nUpSwitchOnThreshold* in the variable [upSwitchOnThreshold](ms-xhelp:///?Id=beckhoff-c35d-4ca0-9bd6-597c2a99a83f) of the DALI control gear.

The output of the DALI control gear is reset as soon as the rising virtual output value reaches or exceeds the threshold value *upSwitchOnThreshold*.

The threshold value is disabled with the value *nUpSwitchOnThreshold* = 255 (MASK).
...
|  |  |
| --- | --- |
| 24476477 | Prior to the DALI command SET UP SWITCH-ON THRESHOLD, the function block sends the DALI command ENABLE DEVICE TYPE 7, as is necessary for application extended commands (see also [FB\_DALI102EnableDeviceType](ms-xhelp:///?Id=beckhoff-6f87-4d1b-b5a6-4f03ac5df1fb)). |

|  |  |
| --- | --- |
| 18961705 | The function block sends the DALI command SET UP SWITCH-ON THRESHOLD twice, as foreseen by the DALI standard for certain commands. |

|  |  |
| --- | --- |
| 36437619 | The function block changes the DTR0 of all connected DALI control gears. |
...
|  |  |
| --- | --- |
| 59503116 | *eCommandPriority* has no function with the KL6811. The command priorities are supported by the KL6821 from the firmware version BD. |

**nUpSwitchOnThreshold:** Threshold value (0…255) for switching off the output with a rising virtual output value.

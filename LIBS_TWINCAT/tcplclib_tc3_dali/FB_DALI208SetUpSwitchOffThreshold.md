# FB_DALI208SetUpSwitchOffThreshold
...
7. FB\_DALI208SetUpSwitchOffThreshold

# FB\_DALI208SetUpSwitchOffThreshold

![34938791](/tcplclib_tc3_dali/1033/Images/gif/10380445835__Web.gif)

This function block saves the value *nUpSwitchOffThreshold* in the variable [upSwitchOffThreshold](ms-xhelp:///?Id=beckhoff-c35d-4ca0-9bd6-597c2a99a83f) of the DALI control gear.

The output of the DALI control gear is reset as soon as the rising virtual output value reaches or exceeds the threshold value *upSwitchOffThreshold*.

The threshold value is disabled with the value *nUpSwitchOffThreshold* = 255 (MASK).
...
|  |  |
| --- | --- |
| 46013667 | Prior to the DALI command SET UP SWITCH-OFF THRESHOLD, the function block sends the DALI command ENABLE DEVICE TYPE 7, as is necessary for application extended commands (see also [FB\_DALI102EnableDeviceType](ms-xhelp:///?Id=beckhoff-6f87-4d1b-b5a6-4f03ac5df1fb)). |

|  |  |
| --- | --- |
| 11469823 | The function block sends the DALI command SET UP SWITCH-OFF THRESHOLD twice, as foreseen by the DALI standard for certain commands. |

|  |  |
| --- | --- |
| 36119548 | The function block changes the DTR0 of all connected DALI control gears. |
...
nUpSwitchOffThreshold   : BYTE := 255;
```
...
|  |  |
| --- | --- |
| 56640484 | *eCommandPriority* has no function with the KL6811. The command priorities are supported by the KL6821 from the firmware version BD. |

**nUpSwitchOffThreshold:** Threshold value (0…255) for switching off the output with a rising virtual output value.

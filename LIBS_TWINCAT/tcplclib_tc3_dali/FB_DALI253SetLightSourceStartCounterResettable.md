# FB_DALI253SetLightSourceStartCounterResettable
...
6. FB\_DALI253SetLightSourceStartCounterResettable

# FB\_DALI253SetLightSourceStartCounterResettable

![38285619](/tcplclib_tc3_dali/1033/Images/gif/8371696267__Web.gif)

The function block sets the resettable counter for the switching on of the light source to any desired value. The resettable counter is incremented by one each time the light source changes from the Off state to the On state.

The resettable counter can be read using the function block [FB\_DALI253QueryLightSourceStartCounterResettable](ms-xhelp:///?Id=beckhoff-b974-4917-b670-a574a2488fa4).
...
nLightSourceStartCounterResettable   : UDINT;
```
...
**eAddressType:** Defines whether the input *nAddress* has a short address (0...63) or a group address (low, middle low, middle, middle high, high). Input *nAddress* has no meaning if a broadcast or a broadcast to unaddressed devices (BroadcastUnaddr) has been selected (see [E\_DALIAddressType](ms-xhelp:///?Id=beckhoff-8a4e-4869-9ea1-1920c00d61f5)).
...
|  |  |
| --- | --- |
| 9026257 | *eCommandPriority* has no function with the KL6811. The command priorities are supported by the KL6821 from the firmware version BD. |

**nLightSourceStartCounterResettable:** New value of the resettable counter for the switching on of the light source (0...16777213).

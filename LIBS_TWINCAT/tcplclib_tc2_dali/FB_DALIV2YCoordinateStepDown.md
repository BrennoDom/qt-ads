...
6. FB\_DALIV2YCoordinateStepDown

# FB\_DALIV2YCoordinateStepDown

![56101816](/tcplclib_tc2_dali/1033/Images/gif/9007200267296395__en-US__Web.gif)

The y-COORDINATE variable is reduced by 256 steps (256 / 65536) without cross-fading.

If the new color value does not correspond to a color that can be achieved by the DALI ballast, this must be indicated by bit 0 of [COLOUR STATUS](ms-xhelp:///?Id=beckhoff-1ce6-4fd4-be82-75082326bd44) (xy-coordinate color point lies outside the valid range). The command is executed only if bit 4 of [COLOUR STATUS](ms-xhelp:///?Id=beckhoff-1ce6-4fd4-be82-75082326bd44) (color representation xy-coordinate active) is set.
...
|  |  |
| --- | --- |
| 35154304 | This command belongs to the application-related expansion commands for lamps for the color/color temperature control with DALI interface. They function only if they are preceded by the *Enable Device Type 8* command, which can be sent with the [FB\_DALIV2EnableDeviceType()](ms-xhelp:///?Id=beckhoff-e277-46a4-85ba-43a8cbcb1cb1) function block. The *Enable Device Type 8* command is internally placed automatically before all application-related expansion commands for lamps for the color/color temperature control with DALI interface. |

...
7. FB\_DALIV2AddressingRandomAddressing

# FB\_DALIV2AddressingRandomAddressing

![18859061](/tcplclib_tc2_dali/1033/Images/gif/9007199398073483__en-US__Web.gif)
...
Applying a positive edge to the *bStart* input starts the function block, and the *bBusy* output goes TRUE. Depending on the selected options (parameter *nOptions*) the group membership and scenarios are subsequently deleted. The function block now addresses all control gears independently. The *arrAddressedDevices* output variable provides information about which control gears have already been allocated a short address. Once all control gears have been addressed, the *bBusy* output switches back to FALSE. Addressing can be aborted through a positive edge at input *bCancel*. Processing this function block can take several minutes, depending on how many control gears are attached.
...
**bCancel:** A rising edge at this input deactivates the function block, thereby interrupting the addressing sequence.
...
| DALIV2\_OPTION\_WITHOUT\_RANDOMISE | The RANDOMISE command is not called before the addressing sequence. This means that all ballasts retain their existing random address ([RANDOM ADDRESS](ms-xhelp:///?Id=beckhoff-1d79-413e-9ba3-da1d3f8e708e)). Only use this option when necessary. |
...
**nCurrentSearchAddress:** Current search address (see [SEARCH ADDRESS](ms-xhelp:///?Id=beckhoff-1d79-413e-9ba3-da1d3f8e708e)).

...
6. FB\_DALIV2SetTemporaryRGBWAFControl

# FB\_DALIV2SetTemporaryRGBWAFControl

![46712854](/tcplclib_tc2_dali/1033/Images/gif/9007200265688331__en-US__Web.gif)

Saves the value in the [TEMPORARY RGBWAF CONTROL](ms-xhelp:///?Id=beckhoff-1ce6-4fd4-be82-75082326bd44) variable of the DALI control gear. The value can be read with the function block [FB\_DALIV2QueryColourValue()](ms-xhelp:///?Id=beckhoff-bf54-40b6-b294-0a21b4b8b625). The *nTemporaryRGBWAFControl* input thereby contains the new assignment (see table).
...
|  |  |
| --- | --- |
| 17762502 | This command belongs to the application-related expansion commands for lamps for the color/color temperature control with DALI interface. They function only if they are preceded by the *Enable Device Type 8* command, which can be sent with the [FB\_DALIV2EnableDeviceType()](ms-xhelp:///?Id=beckhoff-e277-46a4-85ba-43a8cbcb1cb1) function block. The *Enable Device Type 8* command is internally placed automatically before all application-related expansion commands for lamps for the color/color temperature control with DALI interface. |

...
![45251091](/tcplclib_tc2_system/1033/Images/png/9007199285892363__Web.png)

The ADSWRITERES function block is used to acknowledge indications of a PLC task. A response is sent to the ADS source device via a rising edge on the RESPOND input. The source device is addressed via the PORT and NETID parameters. The INVOKEID parameter sorts the responses to the requests for the source device and is adopted by the output of the [ADSWRITEIND](ms-xhelp:///?Id=beckhoff-acb7-4e42-9597-04eceb756e88) function block. An error code can be returned to the ADS source device via the RESULT parameter.
...
**RESULT**: [ADS error code](ms-xhelp:///?Id=beckhoff-102a-43b7-9885-5150d7650de4) or command-specific error code to be sent to the source device.
